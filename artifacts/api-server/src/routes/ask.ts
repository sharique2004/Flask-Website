...
Caseline (ALIX Hackathon winner · voice agent for high stakes admin work). Won the ALIX Agents of Administration hackathon, built solo. A phone agent for administrative work no one can afford to get wrong: callers reach a calm intake line over a real phone number, the browser, or a simulated call, and every path feeds one pipeline. The call lands in a database, Gemini extracts the facts, and each fact is pinned to a verbatim quote from the transcript. Risky actions are never executed by the model; they wait in an approval queue on a live ops console until a human approves. Thesis: An agent for the work no one can afford to get wrong. Stack: Next.js · Vapi voice · Gemini · SQLite. No public demo or repo yet.
